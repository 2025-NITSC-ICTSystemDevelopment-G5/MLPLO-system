'use client';
import { useState } from 'react';
import { useRouter, useSearchParams } from 'next/navigation';

// ★データベースの mock_session とIDを合わせたデータ定義
const CLASSES = [
  {
    id: 1,
    name: 'AIプログラミング体験',
    description: 'Pythonを使って簡単な人工知能を作り、画像認識の仕組みを学びます。初心者でも安心の内容です。',
    pdfLink: '#', // 必要に応じて実在するPDFリンクに変えてください
    sessions: [
       { id: 'sess_001', label: '10:00 - 11:30' }
    ]
  },
  {
    id: 2,
    name: 'IoTロボット制御',
    description: 'M5Stackを使ってセンサーの値を読み取り、モーターを動かす制御プログラミングを体験します。',
    pdfLink: '#',
    sessions: [
       { id: 'sess_002', label: '13:00 - 14:30' }
    ]
  },
  {
    id: 3,
    name: '3D CADデザイン入門',
    description: 'Fusion360を使ってオリジナルのキーホルダーを設計します。3Dプリンタの実演もあります。',
    pdfLink: '#',
    sessions: [
       { id: 'sess_003', label: '15:00 - 16:30' }
    ]
  }
];

export default function SelectClassPage() {
  const router = useRouter();
  const searchParams = useSearchParams();
  
  // URLからIDを取得
  const applicantId = searchParams.get('id');

  const [selectedClass, setSelectedClass] = useState(CLASSES[0]);
  const [selectedSessionId, setSelectedSessionId] = useState(''); // 時間をID(sess_xxx)で管理
  const [loading, setLoading] = useState(false);

  // 申し込み完了処理
  const handleComplete = async () => {
    if (!selectedSessionId) {
      alert("希望の時間を選択してください");
      return;
    }
    setLoading(true);
    
    // APIへ送信
    const res = await fetch('/api/applications', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        applicant_id: applicantId,
        session_id: selectedSessionId // ★選択された時間のIDを送る
      }),
    });

    if (res.ok) {
      alert('授業の予約が完了しました！');
      router.push('/dashboard');
    } else {
      // エラーメッセージを取得して表示
      const errorData = await res.json();
      alert('エラーが発生しました: ' + (errorData.error || '不明なエラー'));
    }
    setLoading(false);
  };

  // --- 画面表示 ---
  return (
    <div style={styles.container}>
      <h1 style={styles.title}>受講したい授業を選択してください</h1>
      <p style={{textAlign: 'center'}}>ID: {applicantId}</p>

      <div style={styles.layout}>
        
        {/* --- 左側：授業ボタン一覧 --- */}
        <div style={styles.leftPanel}>
          <h3 style={styles.subTitle}>授業リスト</h3>
          <div style={styles.buttonList}>
            {CLASSES.map((cls) => (
              <button
                key={cls.id}
                onClick={() => {
                  setSelectedClass(cls);
                  setSelectedSessionId(''); // 授業を変えたら選択時間をリセット
                }}
                style={{
                  ...styles.classButton,
                  backgroundColor: selectedClass.id === cls.id ? '#0070f3' : '#fff',
                  color: selectedClass.id === cls.id ? '#fff' : '#333',
                }}
              >
                {cls.name}
              </button>
            ))}
          </div>
        </div>

        {/* --- 右側：詳細と時間選択 --- */}
        <div style={styles.rightPanel}>
          <h2 style={styles.detailTitle}>{selectedClass.name}</h2>
          
          <div style={styles.descriptionBox}>
            <p>{selectedClass.description}</p>
            <div style={styles.pdfArea}>
              <span>📄 授業詳細PDF: </span>
              <a href={selectedClass.pdfLink} target="_blank" rel="noopener noreferrer" style={styles.link}>
                ダウンロードする
              </a>
            </div>
          </div>

          <h3 style={styles.subTitle}>希望時間を選択</h3>
          <div style={styles.timeList}>
            {selectedClass.sessions.map((sess) => (
              <label key={sess.id} style={styles.radioLabel}>
                <input
                  type="radio"
                  name="timeSlot"
                  value={sess.id}
                  checked={selectedSessionId === sess.id}
                  onChange={(e) => setSelectedSessionId(e.target.value)}
                  style={{ marginRight: '8px' }}
                />
                {sess.label}
              </label>
            ))}
          </div>

          <div style={styles.actionArea}>
            <button 
              onClick={handleComplete} 
              style={styles.completeButton}
              disabled={loading}
            >
              {loading ? '処理中...' : 'この内容で予約する'}
            </button>
          </div>
        </div>

      </div>
    </div>
  );
}

// --- スタイル定義 ---
const styles = {
  container: {
    maxWidth: '1000px',
    margin: '40px auto',
    padding: '20px',
    fontFamily: 'sans-serif',
  },
  title: {
    textAlign: 'center',
    marginBottom: '10px',
  },
  layout: {
    display: 'flex',
    gap: '40px',
    marginTop: '30px',
    minHeight: '400px',
  },
  leftPanel: {
    flex: 1,
    borderRight: '1px solid #ddd',
    paddingRight: '20px',
  },
  buttonList: {
    display: 'flex',
    flexDirection: 'column',
    gap: '10px',
  },
  classButton: {
    padding: '15px',
    textAlign: 'left',
    border: '1px solid #ccc',
    borderRadius: '8px',
    cursor: 'pointer',
    fontSize: '16px',
    transition: '0.2s',
  },
  rightPanel: {
    flex: 2,
  },
  detailTitle: {
    marginTop: 0,
    fontSize: '24px',
    borderBottom: '2px solid #0070f3',
    paddingBottom: '10px',
    marginBottom: '20px',
  },
  subTitle: {
    marginTop: '20px',
    color: '#555',
    fontWeight: 'bold',
  },
  descriptionBox: {
    backgroundColor: '#f9f9f9',
    padding: '20px',
    borderRadius: '8px',
    marginBottom: '20px',
  },
  pdfArea: {
    marginTop: '15px',
    fontWeight: 'bold',
  },
  link: {
    color: '#0070f3',
    textDecoration: 'underline',
  },
  timeList: {
    display: 'flex',
    flexDirection: 'column',
    gap: '10px',
    marginBottom: '30px',
  },
  radioLabel: {
    padding: '10px',
    border: '1px solid #eee',
    borderRadius: '5px',
    cursor: 'pointer',
    fontSize: '18px',
    display: 'flex',
    alignItems: 'center',
  },
  actionArea: {
    textAlign: 'right',
  },
  completeButton: {
    backgroundColor: '#ff4d4f',
    color: 'white',
    padding: '15px 40px',
    fontSize: '18px',
    border: 'none',
    borderRadius: '5px',
    cursor: 'pointer',
    fontWeight: 'bold',
  }
};